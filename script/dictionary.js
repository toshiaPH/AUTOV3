...
module.exports.run = async function({
  ...
  if (!input) {
    return api.sendMessage(global.font("Please provide a word to search for."), event.threadID, event.messageID);
  }
  try {
    ...
    api.sendMessage(global.font(msg), event.threadID, event.messageID);
  } catch (error) {
    if (error.response?.status === 404) {
      api.sendMessage(global.font(`No definitions found for '${word}'.`), event.threadID, event.messageID);
    } else {
      api.sendMessage(global.font("An error occurred while fetching the definition. Please try again later."), event.threadID, event.messageID);
    }
  }
};
