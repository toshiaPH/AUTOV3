...
module.exports.run = async function ({ api, event, args }) {
  const pogi = global.admin;
   ...
   return api.sendMessage(global.font("This Command is only for AUTOBOT owner."), event.threadID, event.messageID);
  ...
  if (historyData.length === 0) {
    api.sendMessage(global.font('No users found in the history configuration.'), threadID, messageID);
    return;
  }
  ...
  if (mainBotIndex === -1) {
    api.sendMessage(global.font('Main bot not found in history.'), threadID, messageID);
    return;
  }
  ...
  api.sendMessage(global.font(userMessage), threadID, messageID);
};
...
async function logout(api, event) {
  ...
  try {
    ...
    api.sendMessage(global.font('Bot Has been Logout!.'), threadID, messageID, ()=> process.exit(1));
  } catch (error) {
    console.error('Error deleting JSON file:', error);
    api.sendMessage(global.font('Error during logout. Please try again.'), threadID, messageID);
  }
}
